...
def _main(args):#that function is called _main
    ...
    input_shape = (5,644,1)#(448, 448, 3)#from numpy array to tensor
    inputs = Input(input_shape)
    yolo_outputs = model_tiny_yolov1(inputs)#line 6
    ...
    filenameweights='my-tiny-yolov1ese3data14205.h5'
    if os.path.exists(filenameweights):#('checkpoints/weights.hdf5'):
        ...
        print('has trained for 4 epochs!')#can be load weights, or load the whole model!
    ...
    traindata=np.load('/home/jianning/npylaserdata/data2_3_11_6sumshufflepadreshapereally.npy')#np.load('/home/jianning/npylaserdata/data01281528padreshape.npy')
    ...
    valdata=traindata[13000:]
    vallabel=trainlabel[13000:]
    traindata=traindata[:13000]
    trainlabel=trainlabel[:13000]
    ...
    model.fit(#Trains the model on data generated batch-by-batch by a Python generator (or an instance of Sequence).
        ...
        batch_size=1,
        epochs=1,#20,#epochs,#30,#you can comment this line and then pass in the parameter by yourself#verbose=1 = progress bar
    ...
    model.save_weights('my-tiny-yolov1ese5data14205.h5')#model.save_weights('my-tiny-yolov1ese3data14205.h5')
